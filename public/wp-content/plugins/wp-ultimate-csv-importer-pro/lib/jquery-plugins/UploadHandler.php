...
class UploadHandler {
	...
	protected function get_upload_path($file_name = null, $version = null) {
		...
		//	$file_name = hash_hmac('md5', "$file_name", 'secret');  // Code added by Fredrick Marks
	...
	protected function trim_file_name($name, $type = null, $index = null, $content_range = null) {
		...
		$current_action = $_REQUEST['current_imptype'];
		$current_blog = $_REQUEST['blog_id'];
                if(in_array('csv' ,$file_name_arr)) {
		if (!is_array($version_zero)) {
			return $file_name_csv[0] . '-' . $current_action . '-' . $current_blog . ' .csv';
		} // Code modified by Fredrick Marks
		else {
			return $file_name_csv[0] . '-' . $current_action . '-' . $current_blog . '-1.csv';
		} // Code modified by Fredrick Marks
		//Edited original file ends
                }
                if(in_array('xml' ,$file_name_arr)) {
                if (!is_array($version_zero)) {
                        return $file_name_xml[0] . '-' . $current_action . '-' . $current_blog . ' .xml';
                } // Code modified by Fredrick Marks
                else {
                        return $file_name_xml[0] . '-' . $current_action . '-' . $current_blog . '-1.xml';
                } // Code modified by Fredrick Marks
                //Edited original file ends
                }
		if(in_array('txt' ,$file_name_arr)) {
		if (!is_array($version_zero)) {
			return $file_name_txt[0] . '-' . $current_action . '-' . $current_blog . ' .txt';
		}
		else{
			return $file_name_txt[0] . '-' . $current_action . '-' . $current_blog . '-1.txt';
		}
		}
                 if($file_name_arr[1] == 'zip') {
                       if (!is_array($version_zero)) {

                        return $file_name_arr[0] . '-' . $current_action . '-' . $current_blog . ' .zip';
                }
                else {
			return $file_name_arr[0] . '-' . $current_action . '-' . $current_blog . '-1.zip';
                     }
                  }
	...
	protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
		$post_url = admin_url() . 'admin.php?page=' . WP_CONST_ULTIMATE_CSV_IMP_SLUG . '/index.php&__module=' . $_POST['curr_action'] . '&import_type=' . $_POST['current_imptype'] . '&step=uploadfile';
		...
		if($file_extension == 'xml'){
			//$xml_file = fopen($uploaded_file,'r');
			//$xml_read = fread($xml_file , filesize($uploaded_file));
        		//fclose($xml_file);
