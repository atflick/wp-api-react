...
if (isset($_POST['upgrade'])) { ?>
        ...
        global $wpdb;
                ...
                $plugin_version = get_option('ULTIMATE_CSV_IMPORTER_UPGRADE_VERSION');
                if(!$plugin_version) {
                        $plugin_version = get_option('ULTIMATE_CSV_IMP_VERSION');
                }
                // Plugin upgradation 
                if($plugin_version != '4.0.1' && $plugin_version != '4.1.0') {
                       delete_option('wp_ultimate_csv_importer_export_exclusion');
                }
...
foreach($acf_choices as $key=>$val){
        $val = serialize($val);
        $sql5 = "insert into smack_field_types(choices,fieldType,groupType)select * from (select '$val','$key','acf-field-type')as tmp where not exists(select groupType from smack_field_types where groupType = 'acf-field-type' and fieldType = '$key')";
        $wpdb->query($sql5);
}
